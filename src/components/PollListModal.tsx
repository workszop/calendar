...
import { Calendar, Clock, Users, CheckCircle2, ChevronRight, Plus, Trash2 } from 'lucide-react';
...
interface PollListModalProps {
  ...
  onOpenNewPoll: () => void;
  /** Asks to delete a poll; the caller confirms before anything is removed. */
  onDeletePoll: (poll: PollSummary) => void;
}
        ...
        <>
          <div className="space-y-2.5">
            {polls.map((p) => {
              ...
              const titleId = `poll-list-title-${p.id}`;
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span id={titleId} className="text-sm font-bold text-stone-900 leading-tight">{p.title}</span>
                        {p.finalizedSlot ? (
                  ...
                  {/* Named "Delete" and described by the title, so a search for the
                      poll's name still finds exactly one button: the row itself. */}
                  <button
                    type="button"
                    aria-label="Delete"
                    aria-describedby={titleId}
                    title="Delete poll"
                    onClick={() => onDeletePoll(p)}
                    className="px-3 border-l border-stone-200 text-stone-400 hover:text-red-600 hover:bg-red-50 transition-colors"
                  >
