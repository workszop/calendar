import type { ParticipantResponse, Poll, SlotStatus } from "../src/types";

// ─── Storage codec ───
// Availability is stored compactly; the API and every caller only ever see the
// verbose `availability` map. A stored participant carries `slots` instead:
...
function decodeSlots(slots: unknown): Record<string, SlotStatus> {
  if (!slots || typeof slots !== "object" || Array.isArray(slots)) {
    throw new Error("Poll store returned participant slots that are not an object.");
  }
  ...
  for (const [date, day] of Object.entries(slots)) {
    if (!DATE_RE.test(date) || typeof day !== "string" || !SLOTS_RE.test(day)) {
      throw new Error("Poll store returned malformed participant slots.");
    }
...
/** The record as stored: participants' availability in compact form where possible. */
  ...
  return {
    ...
    participants: (record.participants as unknown[]).map((raw) => {
      if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
        throw new Error("Poll store returned a participant that is not an object.");
      }
